...
class HybridSystem(eqx.Module):
    ...
    def __init__(self, *, key, width=64, depth=2, init_params=[253, 6347, 84704000, 0.6], r0: float = 5, L0: float = 200):
        key1, _ = jr.split(key)
    ...
    def __call__(self, t, y, u_fn, mf, me):
        ...
        # m_est = 253.641 kg, b_est = 6347.1 kg/s, C_est = 84704000 kPa·mm^3/g, nu_est = 0.6
        x, dx, Pf, Pe = y
        u = u_fn(t)
...
def objective(trial, data_list):
    ...
    width = trial.suggest_int("width", 32, 128, step=16)  # Neural network width
    depth = trial.suggest_int("depth", 1, 4)  # Neural network depth
    init_params = [0.25, 0.65, 0.85, 0.5]

    # Initialize model with trial hyperparameters
    key = jr.PRNGKey(42)
    model = HybridSystem(key=key, width=width, depth=depth, init_params=init_params)
    ...
    n_epochs = 1000  # Reduced for faster hyperparameter search
    for epoch in range(n_epochs):
        model, opt_state, loss = train_step(model, opt_state)
        loss_val = float(loss)

        # Report intermediate loss to Optuna for pruning
        trial.report(loss_val, epoch)

        # Prune trial if it's unlikely to be optimal
        if trial.should_prune():
            raise optuna.TrialPruned()

        if np.isnan(loss_val) or np.isinf(loss_val):
            return float("inf")  # Return high loss for invalid trials
...
def main():
    ...
    study.optimize(lambda trial: objective(trial, data_list), n_trials=10)  # Run 10 trials

    # Print best trial results
    print("\n--- Best Trial ---")
    print(f"Best trial value (loss): {study.best_value:.6f}")
    print("Best hyperparameters:")
    for key, value in study.best_params.items():
        print(f"  {key}: {value}")

    # --- Continue Training with Best Hyperparameters ---
    print("\n🚀 Continuing training with best hyperparameters...")
    ...
    key = jr.PRNGKey(42)
    lr = 2e-2
    #model = HybridSystem(key=key, width=width, depth=depth)
    ...
    resume_training = False  # Set to True if you want to try resuming from a checkpoint
    if resume_training and os.path.exists(model_path) and os.path.exists(opt_state_path):
        ...
        print("🚀 Starting new training with best hyperparameters...")
        best_loss = float("inf")
        best_model = None
        best_opt_state = None
        best_epoch = 0
    ...
    # Print best trial results again
    ...
    Z_flex_nn_p   = jax.vmap(lambda mf_val, x_val: nn_flexion(mf_val, x_val, +5))(M_f.ravel(), X.ravel()).reshape(M_f.shape)
    Z_flex_nn_n   = jax.vmap(lambda mf_val, x_val: nn_flexion(mf_val, x_val, -5))(M_f.ravel(), X.ravel()).reshape(M_f.shape)

    # Compute surfaces for extension (v = +1 and -1)
    Z_ext_nn_p    = jax.vmap(lambda me_val, x_val: nn_extension(me_val, x_val, +5))(M_e.ravel(), X.ravel()).reshape(M_e.shape)
    Z_ext_nn_n    = jax.vmap(lambda me_val, x_val: nn_extension(me_val, x_val, -5))(M_e.ravel(), X.ravel()).reshape(M_e.shape)

    print("Flexion NN +5 range:", Z_flex_nn_p.min(), "to", Z_flex_nn_p.max())
    print("Flexion NN -5 range:", Z_flex_nn_n.min(), "to", Z_flex_nn_n.max())
    print("Extension NN +5 range:", Z_ext_nn_p.min(), "to", Z_ext_nn_p.max())
    print("Extension NN -5 range:", Z_ext_nn_n.min(), "to", Z_ext_nn_n.max())

    # Plot Fig 1: Flexion comparison
    fig = plt.figure()
    ax  = fig.add_subplot(111, projection='3d')
    ax.plot_surface(M_f, X, mask_surface(Z_flex_nn_p), alpha=0.5)
    ax.plot_surface(M_f, X, mask_surface(Z_flex_nn_n), alpha=0.5)
    ax.set_xlabel('mf [g]'); ax.set_ylabel('displacement x [mm]'); ax.set_zlabel('force [mN]')
    ax.legend(['NN v=+5mm/s','NN v=-5mm/s'])
    ...
    fig = plt.figure()
    ax  = fig.add_subplot(111, projection='3d')
    ax.plot_surface(M_e, X, mask_surface(Z_ext_nn_p), alpha=0.5)
    ax.plot_surface(M_e, X, mask_surface(Z_ext_nn_n), alpha=0.5)
    ax.set_xlabel('me [g]'); ax.set_ylabel('displacement x [mm]'); ax.set_zlabel('force [mN]')
    ax.legend(['NN v=+5mm/s','NN v=-5mm/s'])
    ...
    # === Plot Fig 3: The four NN surfaces (flexion & extension at v=±5) ===
    fig = plt.figure()
    ax  = fig.add_subplot(111, projection='3d')
    ax.plot_surface(M_f, X, mask_surface(Z_flex_nn_p), alpha=0.5)
    ax.plot_surface(M_f, X, mask_surface(Z_flex_nn_n), alpha=0.5)
    ...
    ax.set_xlabel('air mass [g]')
    ax.set_ylabel('displacement x [mm]')
    ax.set_zlabel('force [mN]')
    ...
    ax.legend(legend_lines, [
        'Flex NN, v=+5mm/s','Flex NN, v=-5mm/s',
        'Ext NN,  v=+5mm/s','Ext NN,  v=-5mm/s',
    ])
    plt.title('Individual NN Surfaces for Flexion & Extension')
    plt.tight_layout()
    #fig.savefig(os.path.join(checkpoint_dir, f"compare_forcenet_optuna.png"), dpi=150)
